...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            // Every loop iteration, read the IMU (WSpies)
            IMU_read_mult(0x20, sensor_data, 14);
            
            accelX = combine_sensor_data(sensor_data[8], sensor_data[9]);
            accelZ = combine_sensor_data(sensor_data[12], sensor_data[13]);
